...
discount_percent = st.number_input("Discount Percent", min_value=0.0, max_value=100.0)
...
# Show Prediction
    ...
    ax2.bar(['Original Price', 'Discounted Price'], [price, discounted_price])
    st.pyplot(fig2)

    # ---------------- Graph 3 ----------------
    st.markdown("---")
    st.subheader("⭐ Product Popularity Indicators")

    fig3, ax3 = plt.subplots()
    ax3.bar(['Rating', 'Review Count'], [rating, review_count])
    st.pyplot(fig3)